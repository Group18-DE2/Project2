# This script allows to crawl information and repositories from GitHub using the GitHub REST API (https://developer.git>#
...
# To get more than 1,000 elements, the main query should be splitted in multiple subqueries using different time window>#
...
SUBQUERIES = ["+created%3A<%3D2021-4-20","+created%3A>%3D2021-05-24"] #Different subqueries if you need to collect more>PARAMETERS = "&per_page=100" #Additional parameters for the query (by default 100 items per page)
...
for subquery in range(1, len(SUBQUERIES)+1):
        ...
        numberOfPages = int(math.ceil(dataRead.get('total_count')/100))


        #Results are in different pages
        for currentPage in range(1, numberOfPages+1):
                print("Processing page " + str(currentPage) + " of " + str(numberOfPages) + " ...")
                url = URL + QUERY + str(SUBQUERIES[subquery-1]) + PARAMETERS + "&page=" + str(currentPage)             >                dataRead = simplejson.loads(getUrl(url))
